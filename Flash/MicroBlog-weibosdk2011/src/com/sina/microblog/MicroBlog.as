...
//			trace("userAgent	= "+	req.userAgent                         );
		...
		{
			trace("loader_onSecurityError:"+event);
			dispatchEvent(event);
		}
...
//			--BbC04y--
		...
		private var isUseCustomeBrowser:Boolean = false;
		/**
		 * @是否使用用户指定的浏览器AirWebKit，默认为false
		 */ 
		public function set useCustomeBrowser(isUseCustomeBrowser:Boolean):void{
			this.isUseCustomeBrowser = isUseCustomeBrowser;
		}
		///弹出pin登录的授权框
		private function requestAuthorize():void
		{
			...
			if(!isUseCustomeBrowser){
				...
				var e:MicroBlogEvent = new MicroBlogEvent(MicroBlogEvent.OPEN_CUSTOME_BROWSER);
				e.result = url;
				dispatchEvent(e);
			}
		}
